export const fieldSizes = ({ nx, ny, nz }) => ({ h: nx * ny, zeta: nx * ny, temp: nx * ny * nz, salt: nx * ny * nz, u: (nx - 1) * ny * nz, v: nx * (ny - 1) * nz, ubar: (nx - 1) * ny, vbar: nx * (ny - 1), z_r: nx * ny * nz });

export function snapshot(runtime, config) {
  const state = {};
  for (const [id, [name, size]] of Object.entries(fieldSizes(config.grid)).entries()) {
    ...
    try {
      ...
      state[name] = runtime.HEAPF64.slice(pointer / 8, pointer / 8 + size);
      if (state[name].some(value => !Number.isFinite(value))) throw new Error(`${name}: nonfinite ROMS result`);
